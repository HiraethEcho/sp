...
是一般的丰沛 $\mathbb{Q}$-除子。那么  $(X,B+G)$ 和 $(X',B'+H')$  是$W$ (关于 $B_{W}+G_{W}$ 和 $B_{W}+H_{W}$)的两个弱对数典范模型。双标量Sarkisov纲领将构造 $W$ 的有限多个弱对数典范模型 $(X_{i},B_{i}+g_{i}G_{i}+h_{i}H_{i})$，它们是分别关于 $(W,B_{W}+g_{i}G_{W}+h_{i}H_{W})$的模型，其中 $0\leqslant g_i,h_i\leqslant 1$。这些模型间的双有理映射 $\psi_{i}:X_{i}\dashrightarrow X_{i+1}$是通过双射线MMP构造Sarkisov连接映射。
...
\begin{proof}
  ...
  \end{itemize}
  因此 $X_{N}$ 和 $X'$ 都是 $(W,B_{W}+H_{W}+dD_{W})$的对数典范模型，由对数典范模型的唯一性， $X_{N}\cong X'$。更进一步， $f_{N}$ 和  $f'$压缩相同的曲线数值等价类，所以两个森纤维空间同构。
\end{proof}
