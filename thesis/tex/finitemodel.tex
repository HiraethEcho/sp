...
\begin{proof}
  令 $U \subset G(r, V) $ 是不包含任何多面体 $\mathcal{C}_{i} \subset \mathcal{L}_{A}(V)$的面 (face)的$r$-维子空间  $V' \subsetV$的集合。特别的，  $\mathcal{L}_{A}(V')$的内部包含在 $\mathcal{L}_{A}(V)$的内部。显然任何定义在有理数上的子空间 $[V']\in U$ 都满足定理 \ref{mapbetweenAM}的 (1)-(4)。
\end{proof}
\begin{remark}
