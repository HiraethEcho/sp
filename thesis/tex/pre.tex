...
\begin{definition}\cite[Definition 2.3-6]{acc_foliation}
 令 $X$ 为正规代数簇。$X$ 上的一个叶状结构 (folation)是一个凝聚层 $\mathcal{F} \subset \mathcal{T}_{X}$，满足
 \begin{enumerate}
   \item $\mathcal{T}_{X}/\mathcal{F}$是无扭的 (torsion free)；
   \item $\mathcal{F}$在李括号下封闭。 
 \end{enumerate}
令$f:X \dashrightarrow Y$为支配有理映射，将$X$ 上的叶状结构$\mathcal{F}$的在$f$ 的拉回记作  $ f^{-1}(F) $ (叶状结构的拉回的定义，参考\cite[3.1]{acss} 或\cite[3.2]{cs21})，并且称其为$\mathcal{F}$诱导的 $\mathcal{Y}$ 上的叶状结构。令$g:X \dashrightarrow X' $为双有理映射，则将$(g^{-1})^{-1}(\mathcal{F})$记为 $g_{*}\mathcal{F}$，并称为$\mathcal{F}$的推出。

如果存在支配有理映射$h:X \dashrightarrow  Z$和$Y$ 上的由点定义的叶状结构$\mathcal{F}_{Z}$ (即$\mathcal{F}_{Z}=0$，此时此叶状结构的叶子 (leaves)是单点，所以称为由点定义的叶状结构) ，使得  $\mathcal{F}=h^{-1}(\mathcal{F}_{Z})$，则称$\mathcal{F}$为代数可积 (algebraically integrable)的叶状结构，并且称$\mathcal{F}$由$h$ 诱导。 
\end{definition}
