...
\begin{definition}
  ...
  和整体差异数
  \[
    \operatorname{totdiscrep}(X, B) :=\operatorname{inf}\{a(E; X, B) : E \text{是} X \text{之上的除子}\}.
  \]
\end{definition}
...
\begin{lemma}\cite[lemma 3.9.3]{BCHM10} 令 $ \pi:(X,B)\to U $是正规拟射影代数簇间的射影态射，且 $(X, B)$是具有klt 奇点的代数簇对，  $B$ 在 $U$上是大除子。如果哦 $f:X\dashrightarrow Y$是 $U$ 上 弱算术典范模型，那么
  \begin{itemize}
    \item $f$ 是$U$上半丰沛模型；
