...
先比较前两种方法的异同点。相同点是：
\begin{enumerate}
  \item 前两种方法都归纳地构造Sarkisov连接 $X_{i} \dashrightarrow X_{i+1}$，并且每个连接都是用双射线MMP来构造，这样的归纳过程是纲领 (program)。两个纲领的终结条件本质上是一样的 (Noether-Fano-Iskovskikh判别法，定理\ref{nfi}和\ref{nfi2})，即$(X_{N}, B_{N}+H_{N})$和$(X',B'+H')$是相同的弱对数典范模型，而$S,S_{N}$是相同的丰沛模型 (在下降法中应该是$(X',B'+ \frac{1}{\mu'}H')$)。
...
$D_{0}$对应弱对数典范模型$X$ 和丰沛模型$S$，并且压缩态射$\phi:X\to S$ (定理\ref{mapbetweenAM}.(2)给出)是森纤维空间；类似的$D_{1}$ 对应森纤维空间 $\psi:Y\to T$。点 $D_{2},D_{3},D_{4}$对应三个Sarkisov连接 (按定理\ref{2Constructlink})；
\begin{itemize}
  \item $D_{2}$对应
\[ \xymatrix{
                         & W\ar[ld]\ar[rd]          & \\
